...
coordinates_max = 3.0  # maximum coordinate
...
for n_elements in set   # number of elements
   ...
   # initial condition
   initial_condition_Heaveside(x) = ifelse.(x .< 0.5, 1.0, 0.0)
   u0 = initial_condition_Heaveside.(x)
   ...
   function rhs!(du, u, x, t)
      ...
      # boundary flux
      flux_numerical[1, 1] = 1
      flux_numerical[end, end] = 0
   ...
   #exact solution
   u_exact = initial_condition_Heaveside.(x .- tspan[2])
   if(n_elements == 16)
     plot!(vec(x), vec(u_exact), label = "exact solution", legend = :bottomleft)
   end
   # solve
   sol = solve(ode, RDPK3SpFSAL49(); abstol = 1.0e-12, reltol = 1.0e-12, ode_default_options()...)

   plot!(vec(x), vec(sol.u[end]), label = "solution at n_elements=$(n_elements)", legend = :bottomleft, lw = 2)

   diff = abs.(sol.u[end] - u_exact)
   L1 = (dx / 3) * (diff[1] + 4 * sum(diff[2:2:end-1]) + 2 * sum(diff[3:2:end-2]) + diff[end])
   diff_sq = (sol.u[end] - u_exact).^2
   L2_sq = (dx / 3) * (diff_sq[1] + 4 * sum(diff_sq[2:2:end-1]) + 2 * sum(diff_sq[3:2:end-2]) + diff_sq[end])
   L2 = sqrt(L2_sq)
   Linf = maximum(abs.(sol.u[end] - u_exact))
   push!(L1_errors, L1)
   push!(L2_errors, L2)
