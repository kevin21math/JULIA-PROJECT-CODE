...
open("dis_upwind_central_errors.csv", "w") do io
    writedlm(io, [error_header], ',')
end
open("dis_upwind_central_errors.csv", "a") do io
    writedlm(io, error_data, ',')
end

rate_header = ["L1_rate", "L2_rate", "Linf_rate"]
rate_data = [L1_rates L2_rates Linf_rates]
open("dis_upwind_central_rates.csv", "w") do io
    writedlm(io, [rate_header], ',')
end
open("dis_upwind_central_rates.csv", "a") do io
